...
const Marquee: React.FC = () => {
  return (
    <div className="w-full bg-primary py-6 overflow-hidden border-y border-primary">
      <div className="flex whitespace-nowrap marquee-content">
        ...
        {[...CAR_NAMES, ...CAR_NAMES].map((name, index) => (
          <div key={index} className="flex items-center px-8">
            <span className="text-white font-sans font-bold text-18px tracking-widest leading-none">
              {name}
            </span>
            <span className="mx-8 text-accent text-24px">•</span>
          </div>
        ))}
      </div>
    </div>
